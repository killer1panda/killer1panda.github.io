...
new_toolbar = '''
  <div class="topbar" style="position:fixed; top:0; left:0; width:100%; height:60px; background:#18181b; border-bottom:1px solid #27272a; z-index:9999; display:flex; justify-content:space-between; align-items:center; padding:0 20px; color:white;">
    <div style="display:flex; align-items:center; gap:15px; font-family:'Inter',sans-serif;">
      <a href="index.html" class="btn-top" style="text-decoration:none; display:inline-flex; align-items:center; justify-content:center; padding:5px 10px; border-radius:4px; font-weight:500; font-size:12px; background:#27272a; border:1px solid #3f3f46; color:white;">← Home</a>
      <div style="font-weight: 600;">Legacy Master Creator Engine</div>
    </div>
...
new_script = '''
  <script>
    ...
    if (pFrame) pFrame.addEventListener('click', (e) => {
      if (hasPhoto) {
        e.stopPropagation();
    ...
    setTimeout(() => {
      ['.member-name', '.member-title', '.birthday-quote', '.main-wishing-header', '.club-wishes-header'].forEach(sel => {
      ...
      canvas.querySelectorAll('.member-name, .member-title, .birthday-quote, .profile-photo-frame, .main-wishing-header, .logo-header, .club-wishes-header').forEach((el, i) => {
        ...
        function onMove(e) {
          ...
          if (canvas) {
            ...
            const originalLocalLeft = el.offsetLeft - origX;
            const originalLocalTop = el.offsetTop - origY;
            
            let localElLeft = originalLocalLeft + nx;
            let localElTop = originalLocalTop + ny;
            ...
            if (!snappedX) {
              for (const other of otherElements) {
                ...
                const otherTrans = transforms[otherTid] || { x: 0, y: 0 };
